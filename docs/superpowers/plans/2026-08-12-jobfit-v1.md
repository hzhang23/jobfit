...
Note: `database_id` is a placeholder until `wrangler d1 create jobfit` is run. Local development and tests do not read it.
...
```bash
git add src/api src/index.ts test/api
...
- [ ] **Step 12: Build the frontend and typecheck**

Run: `npm run build:web && npm run typecheck`
Expected: `web/dist/index.html` exists, no type errors.
