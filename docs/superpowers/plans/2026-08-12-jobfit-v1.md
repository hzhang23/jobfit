...
**Interfaces:**
- Consumes: nothing
- Produces: `Env` interface with bindings `DB: D1Database`, `JOB_RUN: Workflow`, `ASSETS?: Fetcher` (optional, the binding does not exist until Task 10 builds the frontend), the Workers AI binding `AI: Ai`, and vars `SCORING_MODEL: string`, `TAILORING_MODEL: string`. `LOCAL_USER_ID = 'local-user'`, `getCurrentUser(request: Request): string`, `newId(prefix: string): string`.
...
Three defects found by the whole-branch review after all eleven tasks landed.
All three sat in one seam: the per-run call cap meeting the degraded verdict.

**A healthy run reported `degraded` once postings per run exceeded about 20.**
Cap-refused postings were written as `score_failed` and counted as evidence
that the run could not be trusted, so the app's own quota drove its own trust
verdict. With `max_jobs_per_run: 25` and everything working, the banner read
"Today's results are not trustworthy. 15 of 25 postings produced no judgment at
all." A warning that always fires is a warning nobody reads, and this project's
entire subject is a warning worth reading. Fixed by giving those postings their
own outcome, `not_attempted`, kept out of both sides of the degraded ratio and
reported separately on the receipt. They are deferred, not failed.

**The call cap did not bound calls when calls failed.** `scoringCalls` and
`tailoringCalls` incremented only after a step resolved, so a failing call was
free. During a provider outage, 15 postings produced 15 calls against a cap of
10, and Workflows retries each step three times on top of that. The bound
failed in the one scenario it was written for. Fixed by counting the attempt.

**A run that succeeded reported `failed` after a step retry.** A Workflow step
is at-least-once. A score step that committed its match row and then died left
that row behind, the retry hit `idx_match_job`, the catch tried to insert a
`score_failed` row for the same job and hit it again, and that error escaped
its own step into the outer catch. Fixed with `ON CONFLICT(job_id) DO NOTHING
RETURNING id` in `insertMatch`, falling back to the existing id.

None of the three were reachable from any test as written. Each now has a
regression test, and each was proven to fail with its fix removed.
...
function aiStub(scores: number[]) {
  ...
  // A bare function cast to AiRunner type checks and then throws at runtime,
  // and the "never calls the model" test would pass for the wrong reason.
  return { ai: { run } as unknown as AiRunner, run };
}
...
describe('runPipeline', () => {
  ...
  it('fails the run before calling the model when there is no master resume', async () => {
    ...
    // user-creation function, since users are seeded only by the migration,
    // so a user row for 'nobody' has to exist before createRun will accept it.
...
// `as const` on the whole object, not just on backoff. The workers-types
// definitions type `delay` and `timeout` as template literal types, not as
// plain strings, so a widened string does not satisfy them.
const RETRY = {
    ...
    await runPipeline(
      {
        ...
        // A second, narrower cast than the one on `ai` above. `StepRunner` is
        // intentionally unconstrained so a plain async function can stand in
        // for it in tests. The real `WorkflowStep.do` requires its payload to
        // extend `Rpc.Serializable`, a constraint `StepRunner` does not and
        // should not carry. The cast lives at this single call site.
        step: <T>(name: string, fn: () => Promise<T>) =>
...
- [ ] **Step 13: Add `assets` to `wrangler.jsonc` only, not to `wrangler.test.jsonc`**

Add this key to `wrangler.jsonc`, leaving everything already in the file
unchanged. Do NOT add it to `wrangler.test.jsonc`. The assets directory is a
build artifact that is gitignored, so a test config naming it would abort the
suite on any machine that has not run the frontend build. `test/api/index.test.ts`
already covers the no-assets path deliberately, and `Env.ASSETS` is optional
precisely so this is a supported state rather than a crash.

That makes three fields where the two configs differ on purpose: `ai`,
`workflows`, and now `assets`. Each has its reason written in a comment in
`wrangler.test.jsonc`. Add a comment there for this one too.
...
- [ ] **Step 14b: Add the React render test suite**

The Workers test pool exists to run code inside a Workers runtime. These
components take props and return HTML, so booting one for them buys nothing.
`react-dom/server`'s `renderToStaticMarkup` runs in plain node with no DOM
library at all, which makes a second, much cheaper suite the right shape.

This matters more than it sounds. The frontend is where every product-layer
defense either becomes visible to a user or silently stops mattering, and two
of its states cannot be reached by clicking through the app at all: nothing
links to a `score_failed` match, and nothing links to a passed match with no
tailored resume. Manual verification structurally cannot cover them.

Create `vitest.web.config.ts` at the project root. It must NOT use
`defineWorkersConfig`:
...
Add the matching exclusion to `vitest.config.ts`, or the same files are
collected by both suites and run twice:
...
Update `package.json` so one command runs both, and each is separately
runnable:
...
Create `test/web/match-detail.test.tsx` and `test/web/run-receipt.test.tsx`.
They import the shipped components, never a copy of their markup. `MatchDetail`
fetches in `useEffect`, which server rendering never runs, so its presentational
body is extracted into an exported `MatchDetailView` taking `match` and
`tailored` as props. `MatchDetail` renders it and is its only caller in the app.

The two suites cover, at minimum: a `score_failed` match rendering as an absence
with its reason rather than the string `Score .`; a passed match with no
tailored resume rendering its `outcome_detail`, asserted by rendering two
fixtures that differ only in that field and requiring the HTML to differ; a
score of `0` rendering as a real score; the export defaulting to verified lines
only; and `RunReceipt` across every branch, including each degraded cause
separately and all three together, with an explicit assertion that no fixture
with `scored === 0` cites a highest score.
...
- [Design spec](docs/superpowers/specs/2026-08-12-jobfit-design.md)
- [Failure Mode Map and Tradeoffs Table](docs/IMPACT-Living-Document.md)
...
Expected: all green. `npm test` runs both suites, the Workers one and the React
render one.

Then run: `npx wrangler deploy --dry-run`
Expected: it bundles, and prints `JOB_RUN: JobRunWorkflow` under Workflows,
plus the `AI`, `DB`, and `ASSETS` bindings.

**This step is not optional and it is not a formality.** The test suite runs
against `wrangler.test.jsonc`, which deliberately omits the `ai`, `workflows`,
and `assets` bindings, so nothing in the 120 test suite proves that the real
config and the real entrypoint agree. A typo in the exported class name, a
renamed binding, or a missing entrypoint would pass every test and fail on
deploy. The dry run is the only thing standing in that gap. Treat a dry run
failure as a red build.

Then confirm by hand:
- `grep -rn "—" --include="*.ts" --include="*.tsx" --include="*.md" src web docs README.md` returns nothing
- `git status` is clean and `web/dist` is ignored
- `git ls-files | grep -i -E "\\.dev\\.vars|secret|\\.env"` returns nothing, since this repository is public and must carry no credential file of any kind
