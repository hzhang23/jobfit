...
**Tech Stack:** TypeScript, Cloudflare Workers, Cloudflare Workflows, D1, Hono, React 19, Vite, Vitest with `@cloudflare/vitest-pool-workers`, OpenAI Chat Completions over plain `fetch`.
...
- No OpenAI SDK. Call the HTTP API through `fetch` so the call is trivially stubbable in tests.
- Every external boundary (`fetch` to Jobicy, `fetch` to OpenAI) is injected as a constructor or options parameter defaulting to global `fetch`. Tests never touch the network.
- `screenPosting`, `verifyProvenance`, `passesGate`, and `estimateCost` are pure. No IO, no clock reads, no randomness.
- Secrets live in `.dev.vars` locally and Worker secrets in production. `.dev.vars` is gitignored and must never be committed.
...
**Interfaces:**
- Consumes: nothing
- Produces: `Env` interface with bindings `DB: D1Database`, `JOB_RUN: Workflow`, `ASSETS: Fetcher`, and vars `OPENAI_API_KEY: string`, `SCORING_MODEL: string`, `TAILORING_MODEL: string`, `MONTHLY_COST_CEILING_USD: string`. `LOCAL_USER_ID = 'local-user'`, `getCurrentUser(request: Request): string`, `newId(prefix: string): string`.
...
Then copy it: `cp .dev.vars.example .dev.vars` and paste a real key into `.dev.vars`. `.dev.vars` is already gitignored.
...
```bash
git add package.json package-lock.json tsconfig.json wrangler.jsonc vitest.config.ts .dev.vars.example migrations src test
...
- Produces:
  ...
  - `callOpenAI<T>(opts: CallOptions): Promise<AiResult<T>>` where `CallOptions = { apiKey: string; model: string; system: string; user: string; maxTokens: number; schema?: { name: string; schema: object }; fetchImpl?: typeof fetch }`
  - `interface ScoreResult { score: number; reason: string; evidence: EvidenceItem[] }`
  - `scoreJob(opts: { apiKey: string; model: string; resume: string; job: { title: string; company: string; description: string }; fetchImpl?: typeof fetch }): Promise<AiResult<ScoreResult>>`
  - `tailorResume(opts: { apiKey: string; model: string; resume: string; job: { title: string; company: string; description: string }; fetchImpl?: typeof fetch }): Promise<AiResult<string>>`
...
export interface CallOptions {
  ...
  maxTokens: number;
  /**
   * When present, the model is constrained to this JSON schema. Note that
   * OpenAI strict mode does not support numeric bounds, so range checks live
   * in the caller.
   */
...
export async function callOpenAI<T>(opts: CallOptions): Promise<AiResult<T>> {
...
function stubCompletion(content: unknown, usage = { prompt_tokens: 2000, completion_tokens: 100 }) {
  return vi.fn(async () =>
    new Response(
      JSON.stringify({
...
const valid = {
  ...
  evidence: [{ jdQuote: 'services in Go', resumeQuote: 'rewriting the pricing service in Go' }],
};
...
describe('scoreJob', () => {
  it('returns the parsed score and reports token usage', async () => {
    const result = await scoreJob({
      ...
      resume: 'resume text',
      job,
      fetchImpl: stubCompletion(valid),
    });

    expect(result.data).toEqual(valid);
    expect(result.usage).toEqual({ model: 'gpt-4o-mini', tokensIn: 2000, tokensOut: 100 });
  });

  it('truncates a very long description before sending it', async () => {
    const f = stubCompletion(valid);
    await scoreJob({
      apiKey: 'k',
      model: 'gpt-4o-mini',
      resume: 'resume text',
    ...
    expect(sent.messages[1].content.length).toBeLessThan(8000);
  });

  it('throws on malformed JSON so the workflow step retries', async () => {
    await expect(
      scoreJob({ apiKey: 'k', model: 'gpt-4o-mini', resume: 'r', job, fetchImpl: stubCompletion('not json') }),
  ...
  it('rejects an out of range score instead of storing it', async () => {
    await expect(
      scoreJob({
        ...
        resume: 'r',
        job,
        fetchImpl: stubCompletion({ ...valid, score: 140 }),
      }),
  ...
  it('rejects a score that arrives with no evidence', async () => {
    await expect(
      scoreJob({
        ...
        resume: 'r',
        job,
        fetchImpl: stubCompletion({ ...valid, evidence: [] }),
      }),
  ...
  it('rejects an empty reason', async () => {
    await expect(
      scoreJob({
        ...
        resume: 'r',
        job,
        fetchImpl: stubCompletion({ ...valid, reason: '   ' }),
      }),
...
const job = { title: 'Backend Engineer', company: 'Acme', description: 'Go and Kubernetes.' };
...
describe('tailorResume', () => {
  it('returns the rewritten resume text and usage', async () => {
    const result = await tailorResume({
      ...
      job,
      fetchImpl: stubText('Rewritten resume'),
    });
  ...
  it('sends a system prompt that forbids inventing facts', async () => {
    const f = stubText('ok');
    await tailorResume({ apiKey: 'k', model: 'gpt-4o', resume: 'r', job, fetchImpl: f });

    const sent = JSON.parse(
      ((f as unknown as ReturnType<typeof vi.fn>).mock.calls[0][1] as RequestInit).body as string,
    );
    expect(sent.messages[0].content).toMatch(/never invent/i);
  });

  it('throws when the model returns nothing', async () => {
    await expect(
      tailorResume({ apiKey: 'k', model: 'gpt-4o', resume: 'r', job, fetchImpl: stubText('   ') }),
...
import { type AiResult, callOpenAI } from './client';
 ...
 * `additionalProperties: false` at every level. It does not support numeric
 * bounds, so the 0 to 100 range is enforced in code below.
 */
...
export async function scoreJob(opts: {
  ...
  const result = await callOpenAI<ScoreResult>({
    apiKey: opts.apiKey,
    model: opts.model,
    system: SYSTEM,
    user,
    maxTokens: 800,
    schema: { name: 'job_fit_score', schema: SCORE_SCHEMA },
 ...
 * provenance validator that runs over whatever comes back.
 */
const SYSTEM = [
...
git commit -m "feat: add OpenAI client, job scorer, and resume writer"
```
...
describe('repo', () => {
  ...
  it('saves a tailored resume with its provenance report', async () => {
    ...
    expect(stored).toMatchObject({ content: 'tailored body', unverified_count: 0, model: 'gpt-4o' });
  });

  it('accumulates spend in the ledger and rolls it up for 30 days', async () => {
    await repo.recordUsage(env.DB, { userId: USER, runId, model: 'gpt-4o-mini', purpose: 'score', tokensIn: 2000, tokensOut: 100 });
    await repo.recordUsage(env.DB, { userId: USER, runId, model: 'gpt-4o', purpose: 'tailor', tokensIn: 4000, tokensOut: 1200 });

    expect(await repo.rolling30DayCost(env.DB, USER)).toBeCloseTo(0.02236, 6);
  });

  it('finishes a run with a status and a cost', async () => {
    await repo.finishRun(env.DB, runId, 'degraded', null, 0.04);
    const run = await repo.getRun(env.DB, runId);
    expect(run).toMatchObject({ status: 'degraded', cost_usd: 0.04 });
...
export async function finishRun(
  ...
  await db
    .prepare('UPDATE runs SET status = ?, error = ?, cost_usd = ?, finished_at = ? WHERE id = ?')
    .bind(status, error, costUsd, now(), runId)
    .run();
}
...
export async function recordUsage(
  ...
  const costUsd = estimateCost(u.model, u.tokensIn, u.tokensOut);
  await db
    .prepare(
      `INSERT INTO usage_ledger (id, user_id, run_id, model, purpose, tokens_in, tokens_out, cost_usd, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    )
    .bind(newId('use'), u.userId, u.runId, u.model, u.purpose, u.tokensIn, u.tokensOut, costUsd, now())
    .run();
  return costUsd;
}

export async function rolling30DayCost(db: D1Database, userId: string): Promise<number> {
  const since = now() - 30 * 24 * 60 * 60 * 1000;
  const row = await db
    .prepare('SELECT COALESCE(SUM(cost_usd), 0) AS total FROM usage_ledger WHERE user_id = ? AND created_at >= ?')
    .bind(userId, since)
    .first<{ total: number }>();
  return row?.total ?? 0;
}
...
- Produces:
  ...
  - `runPipeline(deps: PipelineDeps, params: JobRunParams): Promise<void>` where `PipelineDeps = { db: D1Database; source: JobSource; apiKey: string; scoringModel: string; tailoringModel: string; fetchImpl?: typeof fetch; step: StepRunner }`
...
/** Returns a fetch stub that answers scoring calls then tailoring calls. */
function aiStub(scores: number[]) {
  let scoreIndex = 0;
  return vi.fn(async (_url: string, init?: RequestInit) => {
    const body = JSON.parse(init!.body as string) as { response_format?: unknown };
    const isScoring = body.response_format !== undefined;

    const content = isScoring
      ? JSON.stringify({
...
async function deps(source: JobSource, fetchImpl: typeof fetch) {
  return {
    db: env.DB,
    source,
    apiKey: 'k',
    scoringModel: 'gpt-4o-mini',
    tailoringModel: 'gpt-4o',
    fetchImpl,
    step: inlineStep,
  };
}
...
describe('runPipeline', () => {
  ...
  it('scores, gates, tailors, and writes a receipt', async () => {
    ...
    await runPipeline(await deps(source, aiStub([88, 40])), {
      userId: USER,
      trigger: 'manual',
      runId,
    });
  ...
  it('records score_failed for one posting without killing the run', async () => {
    ...
    const flaky = vi.fn(async (_url: string, init?: RequestInit) => {
      const body = JSON.parse(init!.body as string) as { response_format?: unknown };
      if (body.response_format !== undefined && call++ === 0) {
        return new Response('boom', { status: 500 });
      }
      return new Response(
        JSON.stringify({
          choices: [{ message: { content: JSON.stringify({ score: 91, reason: 'ok', evidence: [{ jdQuote: 'Go', resumeQuote: 'Go' }] }) } }],
  ...
  it('skips postings it has already seen', async () => {
    ...
    await runPipeline(await deps(source, aiStub([88])), { userId: USER, trigger: 'manual', runId });

    const second = await repo.createRun(env.DB, USER, 'manual');
    await runPipeline(await deps(source, aiStub([88])), {
      userId: USER,
      trigger: 'manual',
      runId: second,
    });
  ...
  it('fails the run before spending money when there is no master resume', async () => {
...
export async function runPipeline(deps: PipelineDeps, params: JobRunParams): Promise<void> {
  ...
  try {
    ...
    for (const jobId of screened.passIds) {
      try {
        const scored = await step(`score-${jobId}`, async () => {
          ...
          const spend = await repo.recordUsage(db, {
            userId,
            runId,
          ...
          return { matchId, passed, spend };
        });

        costUsd += scored.spend;
    ...
    for (const matchId of passedMatchIds) {
      try {
        const spend = await step(`tailor-${matchId}`, async () => {
    ...
    await step('finalize', async () => {
      ...
      await repo.finishRun(db, runId, degraded ? 'degraded' : 'succeeded', null, costUsd);
    });
  } catch (error) {
    await repo.finishRun(db, runId, 'failed', String(error).slice(0, 1000), costUsd);
  }
}
...
describe('API', () => {
  ...
  it('GET /api/runs/:id returns the receipt', async () => {
    ...
    await repo.finishRun(env.DB, runId, 'succeeded', null, 0.01);
...
runs.post('/runs', async (c) => {
  ...
  const spent = await repo.rolling30DayCost(c.env.DB, userId);
  const ceiling = Number(c.env.MONTHLY_COST_CEILING_USD);
  if (Number.isFinite(ceiling) && spent >= ceiling) {
    return c.json(
      { error: `Rolling 30 day spend is $${spent.toFixed(2)}, at or above the $${ceiling} ceiling` },
      429,
    );
  }
...
runs.get('/runs/:id', async (c) => {
  ...
  return c.json({ ...run, receipt: await repo.getRunReceipt(c.env.DB, run.id) });
});
```
...
export default {
  ...
  async scheduled(event: ScheduledController, env: Env): Promise<void> {
    ...
    const ceiling = Number(env.MONTHLY_COST_CEILING_USD);

    for (const userId of await repo.dueUsers(env.DB, hour)) {
      const spent = await repo.rolling30DayCost(env.DB, userId);
      if (Number.isFinite(ceiling) && spent >= ceiling) {
        console.warn(`Skipping ${userId}: 30 day spend $${spent.toFixed(2)} is at the ceiling`);
        continue;
      }
...
- [ ] **Step 8: Add `main`, `workflows`, and `triggers` to `wrangler.jsonc`**
...
Add these three keys, leaving everything already in the file unchanged:
...
Also change `compatibility_date` in the same file from `2025-09-01` to `2024-12-30`, and add this comment above it:
...
export const api = {
  ...
  listRuns: () => req<Omit<Run, 'receipt'>[]>('/runs'),
...
- [ ] **Step 3: Confirm the API key is set**

Run: `cat .dev.vars`
Expected: a real `OPENAI_API_KEY`. If missing, `cp .dev.vars.example .dev.vars` and paste one in.
...
Requires Node 20 or newer and an OpenAI API key.
...
cp .dev.vars.example .dev.vars    # then paste your OPENAI_API_KEY into it
...
|---|---|---|
| Scoring model | `wrangler.jsonc` vars | `gpt-4o-mini` |
| Tailoring model | `wrangler.jsonc` vars | `gpt-4o` |
| Monthly spend ceiling | `wrangler.jsonc` vars | `20` USD |
